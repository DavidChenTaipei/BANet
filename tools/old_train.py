...
def main():
    ...
    print('cfg.warmup_iters',cfg.warmup_iters)
    ## lr scheduler
    lr_schdr = WarmupPolyLrScheduler(optim, power=0.9,
        max_iter=cfg.epoch*371, warmup_iter=cfg.warmup_iters*371,
        warmup_ratio=0.1, warmup='exp', last_epoch=-1,)
    #lr_schdr = ExponentialLR(optim,gamma=0.99999)
    #lr_schdr = ReduceLROnPlateauPatch(optim, mode='min', factor=0.9998, patience=2)
##########    
    for epoch in range(cfg.start_epoch, cfg.epoch):
        ...
        if True:
            ...
            print('1')
            lr = lr_schdr.get_lr() #get_last_lr()
    #        print('lrlrlr',type(lr))
            print(lr)
            lr = sum(lr) / len(lr)
        ...
        if((epoch+1)==cfg.epoch) :
            ...
            filename = osp.join(cfg.respth,cfg.stor_name)
            filename  = filename+'_final_model'
            state = net.state_dict()
